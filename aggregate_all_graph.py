...
def graph_rlooper_full_seq(plasmid_type, plasmid, expected):
    ...
    if filename:
        with open(filename) as rlooper_probability_file:
            ...
            lines = list(map(float, lines[4:])) #Remove metadata and parse all as floats
            gene_region_probability = lines[2078:3827] if plasmid == "pFC53" else lines[2158:3590]

            pyplot.plot(list(range(1, len(gene_region_probability) + 1)), gene_region_probability, label='Rlooper')
            ...
            return dict(mse=mse, rmsd=rmsd, rsquared=rsquared)
...
def graph_rlooper_gene(plasmid_type, plasmid, expected):
    ...
    if filename:
        with open(filename) as rlooper_probability_file:
            ...
            pyplot.plot(list(range(1, len(gene_region_probability) + 1)), gene_region_probability, label='Rlooper (Gene)')
...
RLOOPER_FILE_FULL_SEQ_MAP = {
    ('pFC53', 'SUPERCOILEDCR'): "pfc53_full_supercoiled_coding_output_bpprob.wig",
    ('pFC53', 'GYRASECR'): "pfc53_full_gyrase_coding_output_bpprob.wig",
    ('pFC53', 'LINEARIZED'): "pfc53_full_linear_coding_output_bpprob.wig",
    ('pFC8', 'GYRASECR'): "pfc8_gyrase_full_output_bpprob.wig",
    ('pFC8', 'SUPERCOILEDCR'): "pfc8_supercoiled_full_output_bpprob.wig",
    ('pFC8', 'LINEARIZED'): "pfc8_linearized_full_output_bpprob.wig",
}
...
RLOOPER_FILE_GENE_MAP = {
    ...
    ('pFC8', 'SUPERCOILEDCR'): "pfc8_gene_supercoiled_output_bpprob.wig",
    ('pFC8', 'LINEARIZED'): "pfc8_gene_linearized_output_bpprob.wig",
}

def aggregate_graph(plasmid_type, plasmid, folder, avg_only: bool = False) -> None:
    ...
    rlooper_full_dict = graph_rlooper_full_seq(plasmid_type, plasmid, probabilities_list)
    ...
    pyplot.legend()
    pyplot.title(f"{plasmid_type} Prediction on {plasmid} (pFC8 $\cup$ pFC53; $n={width}$, $p={padding}$)")
    pyplot.xlabel("Base Position")
    pyplot.ylabel("Probability")

    pyplot.gca().set_ylim(0, 1.0)

    pyplot.savefig(f'PNG_union_{plasmid_type}_{plasmid}_p{padding}_w{width}{"" if not avg_only else "_avg"}.png')
    ...
    return dict(mse=mse, rmsd=rmsd, rsquared=rsquared), rlooper_full_dict, rlooper_gene

WIDTH = 4

if __name__ == "__main__":
    plasmid_types = ["GYRASECR", "LINEARIZED", "SUPERCOILEDCR"]
    table = prettytable.PrettyTable()
    table.field_names = ["Plasmid Type", "Plasmid", "Width", "Padding", "MSE", "RMSD", "RSQUARED"]
    table.align = "l"
        ...
        for plasmid_type in plasmid_types:
            ...
            for plasmid in plasmids:
                for folder in folders:
                    stats = aggregate_graph(plasmid_type, plasmid, folder, avg_only=True)
                    ...
                    if stats[2]:
                        row = [plasmid_type + " (Rlooper Gene)", plasmid, WIDTH, padding]
